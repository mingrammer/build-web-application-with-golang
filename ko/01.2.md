...
$GOPATH에 하나 이상의 경로(작업 공간)를 가지는 것은 괜찮지만 `:`(윈도우에선 `;`)를 사용해서 경로를 나눠줘야 한다는 것을 기억하세요. 이때, `go get`은 $GOPATH의 첫 번째 경로에 내용을 저장합니다. 그래서 여러가지 버전을 같이 사용하지 않는 것이 좋습니다. 최악의 경우는 $GOPATH에서 프로젝트 이름으로 폴더를 생성하는 것입니다. $GOPATH에 폴더를 만들때 패키지를 `<packagename>`과 같이 직접 참조하기 때문에 코드를 변경하는 모든 작업을 할 수가 없으며, 여러분의 패키지를 임포트하는 모든 애플리케이션이 중단됩니다. 왜냐하면 `go get`이 패키지를 그 어디에서도 찾을 수 없기 때문입니다. 그러니 부디 규칙을 따르세요. 규칙이 만들어진 데에는 다 이유가 있습니다.
...
`go get -u …`를 사용하여 원격 패키지를 업데이트 할 수 있으며, 모든 종속 패키지도 자동으로 설치할 수 있습니다.
...
(Windows의 환경 변수 형식은 `%GOPATH%`이지만, 이 책은 주로 Unix 스타일을 따르므로 Windows 사용자는 이를 직접 바꿔야합니다.)
